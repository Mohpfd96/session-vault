...
| ------------------------------ | --------------------------------------------- | -------------------------------------------------------- |
| HTTP `Cookie` request header   | DNR `SET` from virtual jar                    | Strong, if rules are installed before the request        |
| Native `Set-Cookie` absorption | DNR `REMOVE` response `Set-Cookie`            | Strong for network responses DNR can see                 |
| `document.cookie`              | MAIN-world getter/setter                      | Strong for page JS; HttpOnly excluded                    |
| `localStorage`                 | per `(sessionId, origin)` namespace           | Strong for virtualized pages                             |
| IndexedDB                      | database-name prefix; app sees original names | Strong for virtualized `indexedDB.open/delete/databases` |
| Cache Storage                  | cache-name prefix                             | Strong for `caches.*`; **not** HTTP cache                |
| `BroadcastChannel`             | channel-name prefix                           | Strong for constructed channels                          |
| Web Locks                      | lock-name prefix                              | Best-effort; feature-detected                            |
| `sessionStorage`               | **not** namespaced                            | Standard per-tab semantics                               |

Same-session tabs intentionally share jars and namespaced storage. The boundary is **between sessions**, not between every tab.
...
Modes (user-visible, never silent):
...
Detect construction where possible. If a SharedWorker is used, compatibility is at most **LIMITED**. Do not pretend messages are session-scoped unless wrapping can be proven; if it cannot, surface the limitation.
...
Migration is transactional. Failure restores a safe (fail-closed) state. Partial migration is a defect.
...
This avoids first-request leakage. Child tabs inherit the opener’s session by default (`openerTabId` / navigation APIs). Configurable. URL-only guessing after restart is **disabled** unless a heuristic cannot leak credentials (default: no guess).
...
- **FULL** — no SW/SharedWorker detected; virtual surfaces in force.
...
Reasons are specific (“Service Worker controlling this origin”), never “may not work”.
